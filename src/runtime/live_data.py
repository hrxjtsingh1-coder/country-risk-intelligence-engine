"""Runtime collection and validation for the Country Risk Intelligence Engine."""
...
def _session()->requests.Session:
    s=requests.Session(); s.headers.update({"User-Agent":"country-risk-intelligence-engine/1.3 (+https://github.com/hrxjtsingh1-coder/country-risk-intelligence-engine)","Accept":"application/json,text/csv;q=0.9,*/*;q=0.8"})
...
def _wb_indicator(session:requests.Session,code:str,start:int,end:int)->pd.DataFrame:
    r=session.get(WB_URL.format(indicator=code),params={"format":"json","per_page":40000,"date":f"{start}:{end}"},timeout=TIMEOUT); r.raise_for_status(); payload=r.json()
    cols=["country_iso3","country_name","indicator_code","year","value","source"]
    if not isinstance(payload,list) or len(payload)<2 or not payload[1]: return pd.DataFrame(columns=cols)
    rows=[]
    for rec in payload[1]:
        try: y=int(rec.get("date"))
        except (TypeError,ValueError): continue
        rows.append({"country_iso3":str(rec.get("countryiso3code") or "").upper(),"country_name":str((rec.get("country") or {}).get("value") or ""),"indicator_code":code,"year":y,"value":rec.get("value"),"source":"World Bank Indicators API"})
    return pd.DataFrame(rows,columns=cols)

def _fred_dff(session:requests.Session,start:int,end:int)->pd.DataFrame:
    try:
        r=session.get(FRED_URL.format(series="DFF"),timeout=TIMEOUT); r.raise_for_status(); df=pd.read_csv(io.StringIO(r.text))
        if df.empty:return pd.DataFrame()
        dcol="observation_date" if "observation_date" in df.columns else df.columns[0]; vcol="DFF" if "DFF" in df.columns else df.columns[1]
        df[dcol]=pd.to_datetime(df[dcol],errors="coerce"); df[vcol]=pd.to_numeric(df[vcol],errors="coerce"); df=df.dropna(subset=[dcol,vcol]); df["year"]=df[dcol].dt.year.astype(int); df=df[df.year.between(start-1,end)]
        a=df.groupby("year",as_index=False)[vcol].mean().rename(columns={vcol:"value"}); a["value"]=a.value.diff()*100.; a=a[a.year.between(start,end)].copy(); a["country_iso3"]="USA"; a["country_name"]="United States"; a["indicator_code"]="POLICY_RATE_YOY_CHANGE_BPS"; a["source"]="FRED DFF; annual-average change in basis points"
...
def _clean_long(df:pd.DataFrame)->pd.DataFrame:
    ...
    def flag(row):
        if pd.isna(row.value): return "missing"
        lo,hi=bounds.get(row.indicator_code,(None,None))
        return "out_of_range" if (lo is not None and row.value<float(lo)) or (hi is not None and row.value>float(hi)) else "ok"
    out["flag"]=[flag(r) for _,r in out.iterrows()]; out=out.dropna(subset=["country_iso3","indicator_code","year"]); out["year"]=out.year.astype(int)
...
def latest_valid_analysis_year(long:pd.DataFrame,indicators:list[str],min_coverage:float=.55,min_countries:int=30)->int|None:
    ...
    ok=long[long.flag=="ok"]
    if ok.empty:return None
    for y in sorted(ok.year.unique(),reverse=True):
        p=ok[ok.year==y]; countries=p.country_iso3.nunique(); indicator_share=p.indicator_code.isin(indicators).mean() if not p.empty else 0
        if countries>=min_countries and indicator_share>=min_coverage:return int(y)
...
def fetch_live_data(start_year:int=2012,end_year:int|None=None)->tuple[pd.DataFrame,dict[str,Any]]:
    end_year=end_year or pd.Timestamp.utcnow().year; session=_session(); cfg=indicator_records(); frames=[]; status=[]; started=time.time()
    for item in cfg:
        code=str(item["code"])
        try:
            if code=="FX_YOY_DEPRECIATION_PCT": frame=_fx_from_raw(_wb_indicator(session,"PA.NUS.FCRF",start_year-1,end_year),start_year,end_year)
            elif item.get("source")=="fred_us_only": frame=_fred_dff(session,start_year,end_year)
            elif item.get("world_bank"): frame=_wb_indicator(session,str(item["world_bank"]),start_year,end_year); frame["indicator_code"]=code if not frame.empty else frame.get("indicator_code")
            else: frame=pd.DataFrame()
            status.append({"indicator":code,"source":"FRED" if item.get("source")=="fred_us_only" else "World Bank","status":"PASS" if not frame.empty else "NO_DATA","observations":int(len(frame))})
            if not frame.empty: frames.append(frame)
        except Exception as exc:
            LOG.warning("%s failed: %s",code,exc); status.append({"indicator":code,"source":"FRED" if item.get("source")=="fred_us_only" else "World Bank","status":"ERROR","observations":0})
